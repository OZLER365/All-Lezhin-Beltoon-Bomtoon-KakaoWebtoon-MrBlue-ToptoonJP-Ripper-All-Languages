...
(function() {
    ...
    // --- Anti-Debugger Blob Trap Bypass ---
    const OriginalBlob = targetWindow.Blob;
    targetWindow.Blob = function(blobParts, options) {
        if (blobParts && Array.isArray(blobParts)) {
            for (let i = 0; i < blobParts.length; i++) {
                if (typeof blobParts[i] === 'string' && blobParts[i].includes('debugger')) {
                    blobParts[i] = blobParts[i].replace(/debugger;?/g, '');
                }
            }
        }
    ...
    function resetState() {
        ...
        state.urlToHash.clear();
        state.blobHookRegistry.clear();
        ...
        const dlBtn = document.getElementById('lz-dedicated-dl-btn');
        if (dlBtn) {
            dlBtn.innerText = "Download Captured";
    ...
    // --- 1. Helper: Strict Lezhin CDN URL Validator ---
    function isValidHttpImage(url) {
        if (!url || !url.startsWith('https://')) return false;
        return url.includes('rcdn.lezhin') ||
               url.includes('ccdn.lezhin') ||
               url.includes('cdn.lezhin') ||
               url.includes('lezhin.es') ||
               url.includes('lezhinth.com') ||
               url.includes('/contents/scrolls/');
    }

    // --- 2. Total Image Detection ---
    ...
    // --- 4. Decoupled Hook (Populates Fallback Queue Only) ---
    ...
    // --- 5. DOM Scanner (Global Sequence Splicer) ---
    function syncPagesWithDOM() {
        ...
        if (cuts.length > 0) {
            ...
            for (let i = 0; i < cuts.length; i++) {
                ...
                if (!state.pageRegistry.has(pageNum)) {
                    ...
                    if (img && img.src) {
                        ...
                        const isPermittedHttp = isValidHttpImage(img.src);

                        if (isBlob || isPermittedHttp) {
                            if (isBlob) {
                                const bData = state.urlToBlob.get(img.src);
                                if (bData && bData.size < 1024) continue;
                            }
            ...
            if (domImages.length > 0) {
                const getId = (item) => item.isBlob ? (state.urlToHash.get(item.url) || item.url) : item.url;
                let lastFoundIndex = -1;

                for (let i = 0; i < domImages.length; i++) {
                    const currentItem = domImages[i];
                    const currentId = getId(currentItem);
                    if (!currentId) continue;

                    let foundIndex = -1;
                    for (let j = 0; j < state.masterDomList.length; j++) {
                        if (getId(state.masterDomList[j]) === currentId) {
                            foundIndex = j;
                            break;
                        }
                    }
                    ...
                    if (foundIndex !== -1) {
                        ...
                        let insertAt = lastFoundIndex + 1;
                        state.masterDomList.splice(insertAt, 0, currentItem);
                        lastFoundIndex = insertAt;
                        needsUIUpdate = true;
                    }
                }

                if (needsUIUpdate || state.pageRegistry.size !== state.masterDomList.length) {
                    state.pageRegistry.clear();
                    for (let i = 0; i < state.masterDomList.length; i++) {
                        state.pageRegistry.set(i + 1, state.masterDomList[i]);
                    }
                    needsUIUpdate = true;
                }
            } else {
                if (state.pageRegistry.size !== state.blobHookRegistry.size) {
                    state.pageRegistry.clear();
                    for (let [key, val] of state.blobHookRegistry) {
                        state.pageRegistry.set(key, val);
                    }
                    needsUIUpdate = true;
                }
            }
    ...
    // --- 7. Unified Flow: Smart Auto-Scroll ---
    async function smartAutoScroll() {
        // Scroll to top immediately to ensure it grabs from the first page
        window.scrollTo(0, 0);
                ...
                await new Promise(resolve => {
                    ...
                    function check() {
                        ...
                        if (img && img.src) {
                            ...
                            if ((isBlob || isPermittedHttp) && img.complete && img.naturalWidth > 50) {
                                return resolve();
                            }
                        }
        ...
        if (state.isAutoScrolling) {
            ...
            if (state.autoDownload) {
                ...
                setStatus('All panels captured! Click Download Captured.', false);
            }
        }
    }

    // --- 8. Lossless PNG Conversion ---
    ...
    // --- 9. Pure Direct Downloader (PNG Enforcement) ---
    async function downloadToFolder() {
        ...
        const downloadPromises = sortedPages.map(async ([pageNum, item]) => {
    ...
    // --- 10. Unified Background Tick Loop ---
    function updateUI() {
    ...
    // --- 11. Professional Movable & Minimizable UI ---
